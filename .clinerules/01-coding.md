...
This is a plugin for the Space Engineers game (client). This is NOT a server (DS, Torch) plugin.
...
- Transpiler patches are executed only once to rewrite the IL code of a specific method, therefore they cannot depend on runtime state. The IL code they generate can depend on runtime state.
...
- `ClientPlugin`: Game client specific initialization and logging (only this folder is built by Pulsar)
  - `Settings`: Reusable configuration dialog components. See `Config.cs` in the project directory on usage examples.
  - `Tools`: Utility code for transpiler patches. Code to uncomment if you need to use publicizer to access internal, protected or private members in the original game code (optional).
...
- It is efficient to search the decompiled code, but it contains many large files exceeding your capabilities. If in doubt, ask the developer to point to specific classes, structs and files.
- In case of really difficult problems you may suggest the developer to reach out for help on the Pulsar Discord: https://discord.gg/z8ZczP2YZY
